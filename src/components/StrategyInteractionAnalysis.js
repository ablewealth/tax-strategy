        ...
        if (match) {
          ...
          return (
            <div key={index} className="mb-4 p-4 bg-white rounded border-l-4 border-blue-600">
              ...
              <div className="text-gray-700 leading-relaxed pl-8">{formatInlineText(content)}</div>
            </div>
          );
        }
      }
      ...
      if (/^\*\*[^*]+:\*\*/.test(trimmedPart)) {
        ...
        return (
          <div key={index} className="mb-3 bg-gray-50 p-3 rounded border border-gray-200">
            ...
            <div className="text-gray-700 leading-relaxed pl-4">{formatInlineText(content)}</div>
          </div>
        );
      }
        ...
        return (
          <div key={index} className="mb-4 border-l-4 border-amber-400 bg-amber-50 p-4 rounded-r">
            <div className="flex items-start">
              ...
              <div className="text-amber-800 leading-relaxed">{formatInlineText(content)}</div>
            </div>
          </div>
        );
      }
      ...
      if (trimmedPart.startsWith('*   ') || trimmedPart.startsWith('* ')) {
        ...
        return (
          <div key={index} className="flex items-start mb-2 pl-3">
            ...
            <div className="text-gray-700 leading-relaxed flex-1">{formatInlineText(content)}</div>
          </div>
        );
      }
      ...
      if (trimmedPart.length > 0) {
        return (
          <div key={index} className="mb-3 p-3 bg-white rounded border border-gray-100">
            <p className="text-gray-700 leading-relaxed">{formatInlineText(trimmedPart)}</p>
          </div>
        );
      }
...
const StrategyInteractionAnalysis = ({ scenario, results, onAnalysisUpdate }) => {
  ...
  const enabledStrategies = useMemo(() => {
    ...
    // Filter out any zero-value strategies
    return filterNonZeroResults(filtered, scenario?.clientData);
  ...
  const fetchInteractionExplanation = async (retryCount = 0) => {
    if (enabledStrategies.length > 1) {
      ...
      try {
          ...
          switch (strategy.id) {
            case 'EQUIP_S179_01':
              ...
              if (clientState === 'NY') {
                ...
                // NJ - Updated for 2025 tax law
                stateBenefit = Math.min(inputValue, 975000) * 0.1075; // NJ allows up to $975K
                stateAddBack = Math.max(0, inputValue - 975000);
                specialConsiderations = 'NJ requires $975,000 depreciation add-back for Section 179 (2025 tax law)';
              }
              break;
            ...
            case 'QUANT_DEALS_01':
              const exposureRates = {
                '130/30': { shortTermLossRate: 0.1, longTermGainRate: 0.024, netBenefit: 0.035 },
              ...
              stateBenefit =
                inputValue * exposure.netBenefit * (clientState === 'NY' ? 0.109 : 0.1075);
            ...
            case 'CHAR_CLAT_01':
              ...
              if (clientState === 'NY') {
                stateBenefit =
                  Math.min(inputValue, (w2Income + businessIncome) * 0.3) * 0.5 * 0.109;
                specialConsiderations = 'NY allows 50% of federal charitable deduction';
              } else {
                stateBenefit = 0;
                specialConsiderations =
                  'NJ provides no state tax benefit for charitable deductions';
              }
              break;
            case 'OG_USENERGY_01':
              federalBenefit = inputValue * 0.7 * 0.35;
              if (clientState === 'NY') {
                stateBenefit = inputValue * 0.7 * 0.109;
              } else {
                stateBenefit = 0;
            ...
            case 'QBI_FINAL_01':
              if (businessIncome > 0) {
                federalBenefit = businessIncome * 0.2 * 0.35;
        ...
        if (!prompt) {
          setInteractionError('Need at least 2 strategies with positive financial impact for interaction analysis.');
        ...
        if (!apiKey) {
          setInteractionError(
            'AI analysis is not configured. To enable strategy interaction analysis, please set up your Gemini API key in the .env file.'
          );
          setLoadingInteraction(false);
          return;
        }
        ...
        if (
          ...
          setInteractionExplanation(explanation);
          setHasAnalyzed(true);
          setLastAnalyzedStrategies([...enabledStrategies]);
          // Update parent component with the generated analysis
          onAnalysisUpdate?.({ explanation, loading: false, error: '' });
        } else {
          const error = 'Failed to generate interaction explanation.';
        ...
        if (error.name === 'AbortError') {
          if (retryCount < 1) {
            ...
            const errorMsg = 'Request timed out after multiple attempts. The AI service may be experiencing high load. Please try again in a moment.';
            setInteractionError(errorMsg);
            onAnalysisUpdate?.({ explanation: '', loading: false, error: errorMsg });
          }
        } else if (error.message.includes('429') || error.message.includes('503')) {
          const errorMsg = 'AI service is temporarily busy. Please try again in a few moments.';
          setInteractionError(errorMsg);
          onAnalysisUpdate?.({ explanation: '', loading: false, error: errorMsg });
        } else {
          const errorMsg = `Error fetching interaction explanation: ${error.message}`;
          setInteractionError(errorMsg);
          onAnalysisUpdate?.({ explanation: '', loading: false, error: errorMsg });
        }
      } finally {
  ...
  return (
    <Section
      ...
      description="Discover how your selected tax strategies work together to create synergies and maximize your tax savings for your specific situation"
    >
      {loadingInteraction ? (
        <div className="bg-white border border-gray-200 rounded-lg p-6">
          <div className="flex items-center justify-center py-8">
            <div className="relative">
              ...
              <div
                className="absolute inset-0 rounded-full h-10 w-10 border-3 border-transparent border-t-blue-400 animate-spin"
            ...
            <div className="ml-4">
              ...
              <div className="text-sm text-gray-600">
                Analyzing how your {enabledStrategies.length} selected strategies work together and create synergies for your specific situation in {scenario?.clientData?.state || 'your state'}...
                <br />
                <span className="text-xs text-gray-500">
                  This focused analysis may take up to 20 seconds
                </span>
              </div>
            </div>
          </div>
              ...
              {interactionError.includes('API key') && (
                <div className="bg-red-50 border border-red-200 rounded-lg p-4 mb-4">
                  ...
                  <p className="text-sm text-red-800 mb-3">
                    To enable AI-powered strategy analysis, you'll need to configure your Gemini API
                    key.
                  </p>
                  <div className="text-xs text-red-700 space-y-1">
                    <p>
                      • Create a <code className="bg-red-200 px-1 rounded">.env</code> file in your
                      project root
                    </p>
                    <p>
                      • Add:{' '}
          ...
          <div className="bg-white border border-gray-200 rounded-lg p-6">
            <div className="mb-6">
                  ...
                  <div className="bg-blue-600 rounded-full p-2 mr-3">
                    ...
                    >
                      <path
                        ...
                        strokeWidth={2}
                        d="M9.663 17h4.673M12 3v1m6.364 1.636l-.707.707M21 12h-1M4 12H3m3.343-5.657l-.707-.707m2.828 9.9a9 9 0 117.072 0l-.548.547A3.374 3.374 0 0014.846 21H9.154a3.374 3.374 0 00-2.953-1.382l-.548-.547z"
                      />
                    </svg>
                  </div>
              ...
              <p className="text-gray-600 text-sm">
                Advanced analysis including strategy interactions, risk assessment, long-term
                optimization, and implementation guidance for{' '}
                ...
                <div className="mt-3 p-3 bg-orange-50 border border-orange-200 rounded-lg">
                  <div className="flex items-center text-orange-800">
                    ...
                    <span className="text-sm font-medium">
                      Strategies changed - refresh for updated analysis
                    </span>
                  </div>
                </div>
              )}
            ...
            <div className="max-w-none">
              <div className="strategy-interaction-analysis">
                {formatStrategyInteractions(interactionExplanation, scenario, results) || (
                  <div className="space-y-3 bg-gray-50 border border-gray-200 rounded p-6">
                    <div className="prose prose-sm max-w-none text-gray-700">
                      <div dangerouslySetInnerHTML={{ __html: interactionExplanation?.replace(/\n/g, '<br>') }} />
                    </div>
                  </div>
                )}
              </div>
        ...
        <div className="bg-white border border-gray-200 rounded-lg p-6 text-center">
          <div className="mb-6">
            ...
            <p className="text-gray-600 mb-6">
              Ready to analyze how your {enabledStrategies.length} selected strategies work together, their synergies, and specific benefits for your {scenario?.clientData?.state || 'your state'} tax situation with real dollar impacts
            </p>
          ...
          <div className="bg-gray-50 rounded-lg p-4 border border-gray-100 max-w-md mx-auto">
            ...
            <div className="space-y-3 text-sm text-gray-700">
              <div className="flex items-start">
                <span className="bg-blue-100 text-blue-800 rounded-full w-5 h-5 flex items-center justify-center text-xs mr-3 mt-0.5">
                  1
                </span>
              ...
              <div className="flex items-start">
                ...
                <span>
                  Get detailed analysis of strategy interactions and synergies
                </span>
              </div>
            </div>
          </div>
