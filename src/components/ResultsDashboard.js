...
const ResultsDashboard = ({ results }) => {
  ...
  const MetricCard = ({ label, value, subtext, isHighlighted = false }) => (
    <div
      className={`p-4 sm:p-6 rounded-lg text-center transition-all ${isHighlighted ? 'bg-white/20' : 'bg-white/10'}`}
    >
      <h3 className="text-xs font-semibold uppercase tracking-wider opacity-80 mb-2">{label}</h3>
      <p
        className={`text-2xl sm:text-3xl lg:text-4xl font-bold mb-2 ${isHighlighted ? 'text-accent-gold-light' : 'text-white'} leading-tight`}
      >
        {value}
      </p>
  ...
  return (
    <div className="bg-gradient-to-br from-primary-navy to-primary-blue rounded-xl p-6 sm:p-8 lg:p-10 text-white shadow-xl">
      <div className="text-center mb-6 sm:mb-8">
        <h2 className="font-serif text-xl sm:text-2xl lg:text-3xl font-bold leading-tight">
          Executive Tax Optimization Analysis
        </h2>
        <p className="text-sm sm:text-base opacity-80 mt-2">
          Comprehensive strategic tax planning results and optimization metrics.
        </p>
      </div>
      <div className="grid grid-cols-1 sm:grid-cols-2 xl:grid-cols-4 gap-4 sm:gap-6">
        ...
        <MetricCard
          ...
          value={formatCurrency(capitalAllocated)}
          subtext="Total investment in strategies"
        />
      </div>
    </div>
  );
