...
const StrategyCard = ({ strategy, scenario, toggleStrategy, updateClientData, children }) => {
  ...
  return (
    <div
      className={`border rounded-lg p-4 sm:p-6 transition-all relative ${isActive ? 'border-accent-gold bg-gradient-to-br from-white to-amber-50' : 'border-border-primary bg-background-primary hover:border-primary-blue'}`}
    >
      {isActive && (
        <div className="absolute top-0 left-0 w-1.5 h-full bg-accent-gold rounded-l-lg"></div>
      )}
      <div className="flex items-start gap-4">
        ...
        <div className="flex-1 min-w-0">
          <h3 className="font-semibold text-sm sm:text-base text-text-primary leading-tight">
            {strategy.name}
          </h3>
